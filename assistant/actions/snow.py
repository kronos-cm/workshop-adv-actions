...
class SnowAPI:
    ...
    def __init__(self):
        ...
        # Hook into the OS's shutdown signal to asynchronously close the client session
    ...
    async def get_user_profile(self, id: Optional[Text]) -> Dict[Text, Any]:
        ...
        async with session.get(url) as resp:
            if resp.status != 200:
                logger.error(f"Unable to load used profile. Status {resp.status}")
